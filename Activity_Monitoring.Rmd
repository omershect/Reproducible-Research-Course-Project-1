...
qplot(na.omit(Steps_Per_Day)$Total.Steps, geom="histogram",bins=10,xlab = "Steps",ylab = "Count",main = "Total Steps per day Histogram (NA Removed)")  
...
```{r}
#Create a dataframe with  total steps per day  and dates
Steps_Per_Day_NA_Handled<-data.frame(date = unique(as.Date(Activity_DF_NO_NA$date,"%Y-%m-%d")), Total.Steps = tapply(Activity_DF_NO_NA$steps,as.Date(Activity_DF$date,"%Y-%m-%d"),sum))
...
#Plot the Histogram (Remove missing values)
qplot(Steps_Per_Day_NA_Handled$Total.Steps, geom="histogram",bins=10,xlab = "Steps",ylab = "Count",main = "Total Steps per day Histogram (NA Handled)")  
